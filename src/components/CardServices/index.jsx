...
export function CardServices({ titulo, desc }) {
  ...
  return (
    ...
    >
      ...
      {isExpanded ? (
        <MdOutlineKeyboardArrowUp className="absolute top-2 left-2 text-3xl sm:text-4xl lg:text-5xl" />
      ) : (
        <MdOutlineKeyboardArrowDown className="absolute top-2 left-2 text-3xl sm:text-4xl lg:text-5xl" />
      )}
      ...
      <div className="relative text-center text-lg sm:text-xl lg:text-2xl w-full h-full flex items-center justify-center">
        ...
        <p
          className={`text-xs sm:text-lg px-2 sm:px-14 text-justify absolute transition-all duration-500 ease-in-out ${
            isExpanded
